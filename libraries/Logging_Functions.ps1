...
Function Log-Write{
  ...
  .NOTES
    ...
    Version:        1.2
    Author:         John Taylor
    Creation Date:  29/10/14
    Purpose/Change: Added Datetime Stamp and OutScreen Switch
 
  .EXAMPLE
    Log-Write -LogPath "C:\Windows\Temp\Test_Script.log" -LineValue "This is a new line which I am appending to the end of the log file." -DT
  #>
  ...
  Param (
    ...
    [Parameter(Mandatory=$false)][switch]$DT,
    [Parameter(Mandatory=$false)][switch]$OutScreen
    )
  ...
  if($DT){
            ...
            if($OutScreen){
                Write-Host "[$([DateTime]::Now)] $LineValue"
                }
         } else {
...
Function Log-Error{
  ...
  Process{
    Add-Content -Path $LogPath -Value "[$([DateTime]::Now)] Error: An error has occurred [$ErrorDesc]."
  
    #Write to screen for debug mode
    Write-Debug "[$([DateTime]::Now)] Error: An error has occurred [$ErrorDesc]."
